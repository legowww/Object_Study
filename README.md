...
- 디미터 법칙 위반 -> 기차가 충돌된 모양처럼 생겨서 기차 충돌이라고 부르는 코드이다. 일련의 getter()들을 통해 내부 객체들을 참조하는 형태이다.
수신자인 screening에게 내부 구조에 대해 물어보고 있다. 클래스의 내부 구현이 외부로 노출되어 있으며 메세지 전송자가 메세지 수신자의 내부 구현에 강하게 결합된다.
```
```java
